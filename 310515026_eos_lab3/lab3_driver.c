...
{
  uint8_t rec_buf_21[10] = {0};
  uint8_t rec_buf_20[10] = {0};
  uint8_t rec_buf_16[10] = {0};
  uint8_t rec_buf_12[10] = {0};
  
  if( copy_from_user( rec_buf_21, buf, len ) > 0) {
  ...
  if( copy_from_user( rec_buf_20, buf, len ) > 0) {
    pr_err("ERROR: Not all the bytes have been copied from user\n");
  }
  ...
  if( copy_from_user( rec_buf_16, buf, len ) > 0) {
    pr_err("ERROR: Not all the bytes have been copied from user\n");
  }
  ...
  if( copy_from_user( rec_buf_12, buf, len ) > 0) {
    pr_err("ERROR: Not all the bytes have been copied from user\n");
  }
